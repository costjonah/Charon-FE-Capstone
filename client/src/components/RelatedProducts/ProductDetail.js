...
class ProductDetail extends React.Component {
  ...
  showDetail = (product) => {
    if (product) {
      ...
      const productPhoto =
        product && Array.isArray(product.styles) && product.styles[0] && product.styles[0].photos[0].url
      return (
        <div className='detail-card'>
          <div className='card-header'>{this.state.product.name}</div>
          <div className='card-body'>
            <div className='row-group'>{productPhoto && <img src={productPhoto} className='detailPhoto' />}</div>
  ...
  render() {
    return <div className='product-detail-container'>{this.showDetail(this.state.product)}</div>
  }
}
