...
class ProductList extends React.Component {
  ...
  render() {
    const handleRemoveProductFromOutfits = (product) => {
      this.setState({
        outfitProducts: this.state.outfitProducts.filter((item) => item.id !== product.id)
      })
    }
    const handleClickOutFit = (product) => {
      console.log('add', product)
      const exist = this.state.outfitProducts.find((item) => item.id === product.id)
      if (!exist) {
        this.setState({
    ...
    const renderOutFitProducts = () => {
      if (!Array.isArray(this.state.outfitProducts) || this.state.outfitProducts.length <= 0) return null
        ...
        <div className='slide-container'>
          ...
          <div className='card-container'>
            {this.state.productList.map((item, index) => {
              return <ProductCard key={index} product={item} handleClickOutFit={handleClickOutFit} />
            })}
          </div>
